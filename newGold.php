...
";
    ...
    $ur_date = date("Y-m-d");
    $old_user_IP = getIp();
    $user_IP = 'NOT DOING THAT ANYMORE';
    $guestLink = LogMangaGuest();
    if (isset($_POST['RGLTE'])) {
...
$stmtprev = $linkAd->prepare("INSERT INTO eRAmanga.Users_Requests
...
$stmt = $linkAd->prepare("INSERT INTO urequests
                        ( email  , fname        , lname  ,  reqdate , Q1      , Q2   , ltes , 
                          sector      ,  institution , country, `role`   , isStudent  , supEmail, supName , 
                          rothColls   , funding      , ISPG   , agreeCOU , allowEmails, IP , refer,`dlresult`    )
                        VALUES(?, ?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)");
if ( false===$stmt ) {
    die ('prepare() failed: ' . $mysqli->error);
  }
  
$result = $stmt -> bind_param("sssssssssssissssssisss", 
...
$RGrothColls , $RGfunding     , $RGISPG    , $RGagreeCOU , $insertRGallowEmails , $user_IP    , $totRefer, $siteType );
