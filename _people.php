<?php
/**
 * @file _people.php
 * @brief module that lists the people on the project.
 * This displays the people on the project in cards. 
 * This is harcoded html
 * @todo  We could use a list of people in a list json and make a funtion to process that. As the staff is not changing too often, this is not a priority. 
 * @author Nathalie Castells
 * @date 9/27/2018
 */
?>

<div id="idPeople">
	<div class="row p-3 m-3">

		<h2>The e-RA Project Team Today</h2>

	</div>
	<div class="row p-3 m-3">

		<h3>Project Management Team</h3>

	</div>
	<div class="row p-3 m-3">
		<!-- div class="col-sm-6 col-md-4 col-lg-3 col-xl-2 mb-3">
			<div class="card mt-3 text-center border-0">


				<div class="h-100 zoom  zoom">
					<picture>
					<source media="(min-width: 576px)"
						srcset="images/people/Pages/chris-rawlings.jpg">
					<source media="(min-width: 1200 px)"
						srcset="images/people/Thumbs/chris-rawlings.jpg">
					<a target="_blank"
						href="https://www.rothamsted.ac.uk/people/chris-rawlings"><img
						class="card-img-top rounded-circle"
						src="images/people/FullSize/chris-rawlings.jpg"
						alt="Chris Rawlings" style="width: 100%"></a></picture>
				</div>
				<div class="card-body  text-top">
					<h6 class="card-title">
						<a class="text-warning" target="_blank"
							href="https://www.rothamsted.ac.uk/people/chris-rawlings">Chris
							Rawlings</a>
					</h6>
					<p class="card-text small">Chair</p>

				</div>
			</div>
		</div -->
		<!-- div class="col-sm-6 col-md-4 col-lg-3 col-xl-2 mb-3">
			<div class="card mt-3 text-center border-0">
				<div class="h-100 zoom  zoom">
					<picture>
					<source media="(min-width: 576px)"
						srcset="images/people/Pages/malcolm-hawkesford.jpg">
					<source media="(min-width: 1200 px)"
						srcset="images/people/Thumbs/malcolm-hawkesford.jpg">
					<a target="_blank"
						href="https://www.rothamsted.ac.uk/our-people/malcolm-hawkesford"><a
						target="_blank"
						href="https://www.rothamsted.ac.uk/our-people/malcolm-hawkesford"><img
							class="card-img-top rounded-circle"
							src="images/people/FullSize/malcolm-hawkesford.jpg"
							alt="Malcom Hawkesford" style="width: 100%"></a></picture>
				</div>
				<div class="card-body text-top">
					<h6 class="card-title">
						<a target="_blank"
							href="https://www.rothamsted.ac.uk/our-people/malcolm-hawkesford"
							class="text-warning">Malcolm Hawkesford</a>
					</h6>
					<p class="card-text small">Chair</p>

				</div>

			</div>
		</div -->
		<div class="col-sm-6 col-md-4 col-lg-3 col-xl-2 mb-3">
			<div class="card mt-3 text-center border-0">
				<div class="h-100 zoom ">
					<picture>
					<source media="(min-width: 576px)"
						srcset="images/people/Pages/richard-ostler.jpg">
					<source media="(min-width: 1200 px)"
						srcset="images/people/Thumbs/richard-ostler.jpg">
					<a target="_blank"
						href="https://www.rothamsted.ac.uk/people/richard-ostler"><img
						class="card-img-top rounded-circle"
						src="images/people/FullSize/richard-ostler.jpg"
						alt="Malcom Hawkesford" style="width: 100%"> </a></picture>


				</div>
				<div class="card-body text-top">
					<h6 class="card-title">
						<a class="text-warning" target="_blank"
							href="https://www.rothamsted.ac.uk/people/richard-ostler">Richard
							Ostler</a>
					</h6>
					<p class="card-text small">Project Leader</p>

				</div>

			</div>
		</div>
		<div class="col-sm-6 col-md-4 col-lg-3 col-xl-2 mb-3">
			<div class="card mt-3 text-center border-0">
				<div class="h-100 zoom ">
					<picture>
					<source media="(min-width: 576px)"
						srcset="images/people/Pages/nathalie-castells.jpg">
					<source media="(min-width: 1200 px)"
						srcset="images/people/Thumbs/nathalie-castells.jpg">
					<a target="_blank"
						href="https://www.rothamsted.ac.uk/people/nathalie-castells"><img
						class="card-img-top rounded-circle"
						src="images/people/FullSize/nathalie-castells.jpg"
						alt="Nathalie Castells" style="width: 100%"> </a></picture>
				</div>
				<div class="card-body text-top">
					<h6 class="card-title">
						<a class="text-warning" target="_blank"
							href="https://www.rothamsted.ac.uk/people/nathalie-castells">Nathalie
							Castells</a>
					</h6>
					<p class="card-text small">Software Developer</p>

				</div>
			</div>
		</div>
		<div class="col-sm-6 col-md-4 col-lg-3 col-xl-2 mb-3">
			<div class="card mt-3 text-center border-0">
				<div class="h-100 zoom ">
					<picture>
					<source media="(min-width: 576px)"
						srcset="images/people/Pages/margaret-glendining.jpg">
					<source media="(min-width: 1200 px)"
						srcset="images/people/Thumbs/margaret-glendining.jpg">
					<a target="_blank"
						href="https://www.rothamsted.ac.uk/people/margaret-glendining"><img
						class="card-img-top rounded-circle"
						src="images/people/FullSize/margaret-glendining.jpg"
						alt="Margaret Glendining" style="width: 100%"> </a></picture>

				</div>
				<div class="card-body text-top">
					<h6 class="card-title">
						<a class="text-warning" target="_blank"
							href="https://www.rothamsted.ac.uk/people/margaret-glendining">Margaret
							Glendining</a>
					</h6>
					<p class="card-text small">Data curator</p>

				</div>
			</div>
		</div>
		<div class="col-sm-6 col-md-4 col-lg-3 col-xl-2 mb-3">
			<div class="card mt-3 text-center border-0">
				<div class="h-100 zoom ">
					<picture>
					<source media="(min-width: 576px)"
						srcset="images/people/Pages/sarah-perryman.jpg">
					<source media="(min-width: 1200 px)"
						srcset="images/people/Thumbs/sarah-perryman.jpg">
					<a target="_blank"
						href="https://www.rothamsted.ac.uk/people/sarah-perryman"><img
						class="card-img-top rounded-circle"
						src="images/people/FullSize/sarah-perryman.jpg"
						alt="Sarah Perryman" style="width: 100%"> </a></picture>

				</div>
				<div class="card-body text-top">
					<h6 class="card-title">
						<a class="text-warning" target="_blank"
							href="https://www.rothamsted.ac.uk/people/sarah-perryman">Sarah
							Perryman</a>
					</h6>
					<p class="card-text small">Data curator</p>
				</div>

			</div>

		</div>
	</div>
	<div class="row p-3 m-3">

		<h3>Data Provision, Preparation and Analysis</h3>

	</div>

	<div class="row p-3 m-3">
		
		<div class="col-sm-6 col-md-3 col-lg-3 col-xl-2 mb-3">
			<div class="card mt-3 text-center border-0">
				<div class="h-100 zoom ">
					<picture>
					<source media="(min-width: 576px)"
						srcset="images/people/Pages/chris-hall.jpg">
					<source media="(min-width: 1200 px)"
						srcset="images/people/Thumbs/chris-hall.jpg">
					<a target="_blank"
						href="https://www.rothamsted.ac.uk/people/chris-hall"><img
						class="card-img-top rounded-circle"
						src="images/people/FullSize/chris-hall.jpg" alt="Chris Hall"
						style="width: 100%"> </a></picture>

				</div>
				<div class="card-body text-top">
					<h6 class="card-title">
						<a class="text-warning" target="_blank"
							href="https://www.rothamsted.ac.uk/people/chris-hall">Chris
							Hall</a>
					</h6>
					<p class="card-text small">Field Technician</p>

				</div>
			</div>
		</div>
		
		<div class="col-sm-6 col-md-3 col-lg-3 col-xl-2 mb-3">
			<div class="card mt-3 text-center border-0">
				<div class="h-100 zoom ">
					<picture>
					<source media="(min-width: 576px)"
						srcset="images/people/Pages/andy-gregory.jpg">
					<source media="(min-width: 1200 px)"
						srcset="images/people/Thumbs/andy-gregory.jpg">
					<a target="_blank"
						href="https://www.rothamsted.ac.uk/people/andy-gregory"><img
						class="card-img-top rounded-circle"
						src="images/people/FullSize/andy-gregory.jpg"
						alt="Andy Gregory" style="width: 100%"> </a></picture>

				</div>
				<div class="card-body text-top">
					<h6 class="card-title">
						<a class="text-warning" target="_blank"
							href="https://www.rothamsted.ac.uk/people/andy-gregory">Andy
							Gregory</a>
					</h6>
					<p class="card-text small">Project leader</p>

				</div>

			</div>
		</div>
		<div class="col-sm-6 col-md-3 col-lg-3 col-xl-2 mb-3">
			<div class="card mt-3 text-center border-0">
				<div class="h-100 zoom ">
					<picture>
					<source media="(min-width: 576px)"
						srcset="images/people/Pages/tony-scott.jpg">
					<source media="(min-width: 1200 px)"
						srcset="images/people/Thumbs/tony-scott.jpg">
					<a target="_blank"
						href="https://www.rothamsted.ac.uk/people/tony-scott"><img
						class="card-img-top rounded-circle"
						src="images/people/FullSize/tony-scott.jpg" alt="Tony Scott"
						style="width: 100%"> </a></picture>

				</div>
				<div class="card-body text-top">
					<h6 class="card-title">
						<a class="text-warning" target="_blank"
							href="https://www.rothamsted.ac.uk/people/tony-scott">Tony
							Scott</a>
					</h6>
					<p class="card-text small">Environmental Scientist</p>

				</div>
			</div>

		</div>

		<div class="col-sm-6 col-md-3 col-lg-3 col-xl-2  mb-3">
			<div class="card mt-3 text-center border-0">
				<div class="h-100 zoom ">
					<picture>
					<source media="(min-width: 576px)"
						srcset="images/people/Pages/suzanne-clark.jpg">
					<source media="(min-width: 1200 px)"
						srcset="images/people/Thumbs/suzanne-clark.jpg">
					<a target="_blank"
						href="https://www.rothamsted.ac.uk/people/suzanne-clark"><img
						class="card-img-top rounded-circle"
						src="images/people/FullSize/suzanne-clark.jpg" alt="Suzanne Clark"
						style="width: 100%"> </a></picture>

				</div>
				<div class="card-body text-top">
					<h6 class="card-title">
						<a class="text-warning" target="_blank"
							href="https://www.rothamsted.ac.uk/people/suzanne-clark">Suzanne
							Clark</a>
					</h6>
					<p class="card-text small">Statistician</p>

				</div>

			</div>

		</div>
		<div class="col-sm-0 col-md-0 col-lg-0 col-xl-2 mb-3"></div>
	</div>
</div>